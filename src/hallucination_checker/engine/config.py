...
GEHD_VERSION_HASH = "v012-docs-aiguide"
...
class GEHDConfig:
    ...
    # --- L2 黑名单 ---
    blacklist: tuple[str, ...] = field(default_factory=lambda: (
        "母丑", "母丑购", "母丑京东", "母丑商城",
    ))

    # --- L3 实体提取正则 ---
    entity_patterns: tuple[tuple[str, str, int], ...] = field(default_factory=lambda: (
        (r'([\u4e00-\u9fff]{1,3}(?:购|商城|超市|百货|优选|特卖|专营|官方店|旗舰店))', '电商平台名', 60),
        (r'(?:^|[^\u4e00-\u9fff])([\u4e00-\u9fff]{2,6}(?:公司|集团|企业|科技|股份|有限|控股|投资|基金|银行|保险|证券|信托))', '公司机构名', 50),
        (r'(?:^|[^\u4e00-\u9fff])([\u4e00-\u9fff]{2,5}(?:大学|学院|学校|研究院|研究所|实验室|医院|中心|协会|学会|组织))', '学术机构名', 45),
        (r'\b([\u4e00-\u9fff]{2,5}(?:牌|系列|型号|版))\b', '产品品牌名', 40),
        (r'(?:^|[^\u4e00-\u9fff])([\u4e00-\u9fff]{2,6}(?:局|委员会|办公厅|管理处|监管局|指挥部|办公室))', '政府机构名', 48),
        (r'(?:^|[^\u4e00-\u9fff])([\u4e00-\u9fff]{3,8}(?:行业协会|联合会|促进会|联盟|商会|公会))', '行业组织名', 45),
        (r'(?:^|[^\u4e00-\u9fff])([\u4e00-\u9fff]{2,6}(?:微电子|半导体|集成电路|光电子))', '半导体企业名', 50),
        (r'([A-Z][A-Za-z0-9]*[-–][A-Z0-9][A-Za-z0-9]*)', '产品型号', 42),
        (r'(?:^|[^\u4e00-\u9fff])([\u4e00-\u9fff]{2,3})(?:先生|女士|小姐|博士|教授|老师|工程师|经理|总监|总|董)', '人名+称谓', 35),
        (r'(?:^|[^\u4e00-\u9fff]|位于|在|于|至|往|从|的)([\u4e00-\u9fff]{2,4}(?:省|市|县|区|镇|乡|村|路|街|道|广场|大厦|写字楼|商场|机场|车站|港口))', '地名', 30),
        (r'["\u300c\u300d\u201c\u201d]([\u4e00-\u9fff]{2,10})(?:计划|行动|工程|倡议|战略|峰会|论坛|大会|博览会)["\u300c\u300d\u201c\u201d]', '会议/IP名', 32),
        (r'["\u300c\u300d\u201c\u201d]([\u4e00-\u9fff]{2,8})["\u300c\u300d\u201c\u201d]', '引用名称/IP名', 25),
        (r'\b([A-Z][a-z]{2,}(?:[ -][A-Z][a-z]{2,}){1,2})(?: Inc| Corp| LLC| Ltd| Co| Group)?\b', '英文机构/品牌名', 40),
    ))

    # --- L2.5 非实体检测正则 ---
    l25_patterns: tuple[tuple[str, str, int], ...] = field(default_factory=lambda: (
        (r'(\d+(?:\.\d+)?(?:万亿|亿|万)?(?:元|美元|人民币|欧元))', '可疑统计金额', 48),
        (r'(?:增长率?|增速|增幅|涨幅|同比|环比|市占率?|占有率|渗透率|转化率|复购率)[超达]?\s*(\d+(?:\.\d+)?%)', '可疑百分比数据', 45),
        (r'(?<!已)(?<!完成)(?:达|约|将近|超过|突破)\s*(\d+(?:\.\d+)?(?:万亿|亿|万)?\s*(?:元|美元|人|户|家|台|套))', '可疑规模描述', 42),
        (r'据\s*([\u4e00-\u9fff]{2,4}(?:教授|博士|院士|CEO|总裁|总[经理裁]|部长|司长|局长|主任|所长))\s*(?:称|表示|透露|指出|强调|透露|宣布)', '权威引述', 50),
        (r'["\u201c]((?:(?:[\u4e00-\u9fff]{1,3}(?:教授|博士|院士|CEO|总裁|总[经理裁]|部长|司长|局长|主任|所长|先生|女士))'
         ...
         r').{0,50}[^"\u201d]{5,})["\u201d]', '直接引语(待核实)', 35),
        (r'(?:于|在|预计|将于|将在|计划于)\s*(20\d{2}年(?:[一二三四]?季度|Q[1-4]|[1-12]月))', '时间线引用', 38),
        (r'(?:已于?|已在|已经完成|已完成|成功实现)\s*(20\d{2}(?:年[1-12]月|[/-]\d{1,2}))', '完成时间声明', 40),
    ))

    # --- L2.5 排除短语 ---
    l25_exclude_phrases: frozenset[str] = field(default_factory=lambda: frozenset({
        "GDP", "CPI", "PMI", "GDP增长率",
        "2026年", "2025年", "2024年", "2023年",
    }))

    # --- 噪音前缀 ---
    noise_prefixes: tuple[str, ...] = field(default_factory=lambda: (
        '在', '的', '是', '有', '被', '从', '向', '对', '为', '由',
        '把', '与', '和', '及', '或', '而', '但', '若', '如', '因',
    ))

    # --- 排除词 ---
    entity_suffixes_for_exclusion: tuple[str, ...] = field(default_factory=lambda: (
        '中心', '公司', '集团', '研究院', '研究所', '实验室',
        '协会', '学会', '管理局', '委员会', '办公室',
    ))

    exclude_words: frozenset[str] = field(default_factory=lambda: frozenset({
        "采购", "采购完毕", "采购日", "采购建议",
        "购置", "购买",
        "弹力网", "电网", "水电网", "断网", "力网",
        "上网", "在电商平台",
    ...
    # --- 形容词前缀 ---
    adjective_prefixes: frozenset[str] = field(default_factory=lambda: frozenset({
