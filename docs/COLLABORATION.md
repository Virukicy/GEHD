...
正式传递写入 `~/.workbuddy/` 目录（引擎组）或 `.workbuddy/` 目录（UI 组），**文件名即传递号**：

| 方向 | 文件名 | 传递号 | 写入位置 |
|------|------|------|------|
| 引擎→UI | `E2U-YYYYMMDD-NNN.md` | 同文件名 | `~/.workbuddy/` |
| UI→引擎 | `U2E-YYYYMMDD-NNN.md` | 同文件名 | `.workbuddy/` |

**规则**：
- 每次传递**新建文件**，不复用旧文件名
- 序号 NNN 按当天同方向递增（001, 002, ...）
- 文件名即传递号，避免内容与实际文件不匹配
- 回复对方时在正文注明 `回复 {原传递号}`

**示例**：
- 引擎组首次发送 → `~/.workbuddy/E2U-20260509-001.md`
- UI 组回复        → `.workbuddy/U2E-20260509-001.md`，正文注明「回复 E2U-20260509-001」
- 引擎组再次发送   → `~/.workbuddy/E2U-20260509-002.md`
