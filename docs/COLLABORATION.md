...
> PM 决策 → 助理出方案 → PM 审批 → 下发 PM2{执行组}（指令+验证标准）+ 同步 PM2QA（验证指令）→ {执行组}回执 → QA 独立验证 → Q2PM → PM 审阅 → PM2A 闭合

操作原则：
- **无检测不管理**：PM 的每条 A 类指令必须内嵌验收标准。标准按组适配，但结构统一——**执行什么操作 → 期望什么结果 → 如何验证**（可执行命令）。PM 不是分配任务的角色，是定义「完成」的角色（验收标准草案由 S 在 S2PM 中提供，PM 承接补充。）
- {执行组} 按本轮实际需求填入（D / E / U / E+U），不是每次全动
- 有依赖关系的指令按顺序下发（如 E 建目录 → D 改协议 → 其他人迁移）
- 无依赖关系的指令并行下发
- 各组只写自己文件域，不同时操作同一文件则零冲突
- 执行组回执后，PM 发出预制的 PM2QA 启动独立验证；收到 Q2PM 后发 PM2A 宣布闭环
- **S 参与验收设计**：S 在 S2PM 中为每项修订附带验收标准草案（可执行验证命令 + 期望输出）。PM 审阅补充后写入 PM2D 的「验收标准」节。验收标准自 S2PM 阶段启动，非 PM2D 阶段临时追加。
- **模型切换后验证身份卡**：PM 切换模型（如 Pro→Flash）后，首条消息应附带身份卡，并验证执行组是否识别了自身角色。如执行组行为偏离角色定义，重新注入身份卡。

验收标准按组适配（示例）：

| 组 | 验收命令类型 | 典型验证项 |
|:--:|------|------|
| E | `pytest`、`mypy`、`ruff`、`python -c "import ..."`、行为断言 | 新增测试通过、类型无误、功能正确、配置生效 |
| U | `pytest test_gui.py`、UI 截图对比、组件渲染验证 | GUI 测试通过、组件正常渲染、交互流程正确 |
| D | `grep`、`ls`、`git diff --stat`、文件存在性 | 旧措辞清除、新措辞到位、文件迁移正确、结构完整 |

QA 理解的验收标准不关心具体工具——只关心结构。只要 PM2QA 里给出了 **期望值和验证命令**，QA 就能独立运行并输出结论。
...
QA 是独立验证的第一责任方。PM 保留在以下情况自行运行验证命令的权利：
- Q2PM 结论与预期严重不符
- 怀疑 QA 偏离了 PM2QA 中指定的验证标准

若 PM 交叉验证结果与 Q2PM 不一致，Q2PM 视为无效，发回 QA 重做。
...
GEHD 执行智能体（E/U/QA/D）使用 Flash 模型。模型切换时上下文可能断层，身份卡确保角色定义在新会话中不丢失。身份卡的作用是划边界，不是补记忆。**PM 每次调度必须携带身份卡**。
...
### 7.10 合规审查（CI）— 设计保留，暂不激活

CI 角色详细设计见 `docs/designs/agents/ci-role-design.md`。

CI 检查流程是否合规（格式、链完整、文件域），不检查产物质量（由 QA 负责）。触发方式：PM 手动或每 5 个 PM2A 自动。CI 无执行权，升级权在 PM。

当前搁置原因：GEHD 轮数少、PM 亲力可覆盖。进入多支线高频迭代后再评估激活。
...
## 验收标准
{逐项验证命令 + 期望输出，按组适配：
  — E: pytest/mypy/ruff + 行为断言
  — U: GUI 测试 + 组件验证
  — D: grep/ls/git diff + 内容/结构断言}
...
**PM → QA（PM2Q）— A 类指令（独立验证）**：

```
## 背景
（引用 PM2{执行组} 和 {执行组}2PM，说明验证对象）

## 验证指令
{从 PM2{执行组} 的「验收标准」复制验证步骤，加上 QA 额外检查项}
