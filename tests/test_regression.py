...
class TestL36ConsistencyCheck:
    ...
    def test_l36_detects_high_freq_entity(self, check_result):
        ...
        has_consistency = (
            check_result.has_warning_containing('一致性')
            or check_result.has_warning_containing('高频')
            or check_result.has_warning_containing('辰星微电子')
        )
        assert has_consistency, 'FAIL: [L3.6] 高频实体未触发一致性警告 — 一致性检查模块可能失效'
...
class TestL4QueueOutput:
    ...
    def test_l4_queue_has_entities(self, verify_result):
        ...
        assert verify_result.l4_queue is not None, 'FAIL: [L4] verify模式未返回l4_queue'
        assert len(verify_result.l4_queue) > 0, 'FAIL: [L4] 队列为空(应有候选词)'

    def test_l4_source_layer_field(self, verify_result):
        """TC11: 所有L4队列项都有 source_layer 字段 (v3.6新增)"""
        for item in verify_result.l4_queue:
            assert 'source_layer' in item, f'FAIL: [L4] 缺少source_layer字段: {item}'
    ...
    def test_l4_layer_distribution(self, verify_result):
        ...
        assert len(l25_items) >= 2, f'FAIL: [L4] L2.5源项目太少({len(l25_items)})，L2.5层可能失效'
        assert len(l3_items) >= 10, f'FAIL: [L4] L3源项目太少({len(l3_items)})，L3层可能失效'
...
class TestL4ProtocolStructure:
    ...
    def test_l4_json_protocol_fields(self, verify_result, l4_queue_file):
        ...
        with open(l4_queue_file, encoding='utf-8') as f:
        ...
        # 检查版本号
        assert protocol['gehd_version'] == '0.3.0-alpha', (
            f'[L4协议] 版本号应为0.3.0-alpha，实际为{protocol["gehd_version"]}'
        )
...
class TestEdgeCases:
    ...
    def test_version_string_in_output(self, check_result):
        """输出报告应包含当前版本号"""
        assert 'v0.3.0-alpha' in check_result.output, (
            'FAIL: 输出报告中未显示版本号v0.3.0-alpha — 版本常量可能未更新'
        )
