...
class TestL4ProtocolStructure:
    ...
    def test_l4_json_protocol_fields(self, verify_result, l4_queue_file):
        ...
        # 检查版本号
        assert protocol['gehd_version'] == '0.4.0-alpha', (
            f'[L4协议] 版本号应为0.4.0-alpha，实际为{protocol["gehd_version"]}'
        )
...
class TestEdgeCases:
    ...
    def test_version_string_in_output(self, check_result):
        """输出报告应包含当前版本号"""
        assert 'v0.4.0-alpha' in check_result.output, (
            'FAIL: 输出报告中未显示版本号v0.4.0-alpha — 版本常量可能未更新'
        )
